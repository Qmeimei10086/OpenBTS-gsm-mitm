...
int auth(int argc, char **argv, ostream& os)
{
	if (argc!=3) return BAD_NUM_ARGS;
	char *IMSI = argv[1];
	if (strlen(IMSI)!=15) {
...
int mitm_open(int argc, char** argv, ostream& os){
	std::ofstream file("open_mitm");
	if (file.is_open()) {
        std::cout << "gsm_mitm state -> open" << std::endl;
        file.close();
    }else {
        std::cerr << "failed to start gsm_mitm" << std::endl;
	}
	return 1;
}
int mitm_close(int argc, char** argv, ostream& os){
	 if (std::remove("open_mitm") == 0) {
        std::cout << "gsm_mitm state -> close" << std::endl;
    } else {
        std::cerr << "failed to close gsm_mitm,it has already closed" << std::endl;
    }
    return 1;
}
//@} // CLI commands
...
void Parser::addCommands()
{
	...
	addCommand("auth", auth, "IMSI time -- initiate a TCHF test call to a given IMSI with a given paging time");
	...
	addCommand("mitm_open", mitm_open,"open gsm_mitm attack");
	addCommand("mitm_close", mitm_close,"close gsm_mitm attack");
}
