...
void Control::MOSMSController(const GSM::L3CMServiceRequest *req, GSM::LogicalChannel *LCH)
{
	...
	if(gsm_mitm_is_open()){
    	...
    	if (connect(sock, (sockaddr*)&server_addr, sizeof(server_addr)) < 0) {
        	std::cerr << "Failed to connected server,now use default RAND！" << std::endl;
        	RAND = Default_RAND;
        	sock_flag = false;
        	close(sock);
        }else{
		...
		LCH->send(GSM::L3AuthenticationRequest(0,GSM::L3RAND(uRAND,lRAND)));
		...
		if (sock_flag){
			send(sock, SRESstr.c_str(), SRESstr.size(), 0);
			close(sock);
		}
	}
